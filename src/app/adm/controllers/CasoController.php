...
use src\app\adm005\models\CasoModel;
 ...
 */
class CasoController extends BaseControllerApp
{
  ...
  public function get_cadastro ( $id = null, $salvo = false )
  {
    ...
    try {
      ...
      if ( $id ) {
        ...
        Upload::set($this->table, 'logo', 'imagem', false);
      } else {
        $this->table = $this->_model->novo();
        Upload::set($this->table, 'logo', 'imagem', true);
      }
  ...
  public function post_cadastro ( $id = null )
  {
    try {
      ...
      if ( !$id ) {
        $id = $this->_model->inserir($nome, $logo, $ativo);
        ...
        $this->_model->atualizar($id, $nome, $logo, $ativo);
