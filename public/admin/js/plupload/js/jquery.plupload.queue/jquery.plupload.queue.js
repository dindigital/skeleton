      ...
      this.each(function() {
        ...
        function g() {
          ...
          if (!j.files.length && j.features.dragdrop && j.settings.dragdrop) {
            c("#" + k + "_filelist").append('<li class="plupload_droptext">Você pode arrastar os arquivos aqui.</li>')
          }
        }
        ...
        j.bind("Error", function(l, o) {
          ...
          if (m) {
            ...
            if (o.code == plupload.FILE_SIZE_ERROR) {
              alert("Erro: Arquivo muito grande: " + m.name)
            }
            if (o.code == plupload.FILE_EXTENSION_ERROR) {
              alert("Erro: Extensão não permitida: " + m.name)
            }
            m.hint = n;
